...
class Braco:
	def __init__(self, width, height, draw, line):
		...
		self.pos_move  = [400 , height//2 + self.r1]
		self.pos_print = [400 , height//2 + self.r1+self.r2]
	...
	def atualiza(self, draw, line, delete, s1, s2):
		...
		draw(self.pos_print[0]-2, self.pos_print[1]-2, 
				self.pos_print[0]+2, self.pos_print[1]+2, fill = 'red', tag = 'move')
